...
export function addPost(title: string, text: string, author: string): boolean {
	// Normally it's not advised to hard-code all your data!
	// in the absence of a true Model layer, our service can simply return a hard-coded array of posts

	const newPost = {
		id: (PostDatabase.length + 1).toString(),
		title,
		text,
		author:
			getAllUsers().find((user) => user.name === author) || getAllUsers()[0],
	};

	PostDatabase.push({ ...newPost });

	if (PostDatabase.find((post) => post.id === newPost.id) === undefined) {
		return false;
	}
